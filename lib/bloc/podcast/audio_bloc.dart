...
enum TransitionState { play, pause, stop, fastforward, rewind }

enum LifecycleState { pause, resume, detach }
...
class AudioBloc extends Bloc {
  ...
  void _handlePlayingStateTransitions() {
    _transitionPlayingState.asyncMap((event) => Future.value(event)).listen((
      state,
    ) async {
      switch (state) {
  ...
  ValueStream<PositionState>? get playPosition =>
      audioPlayerService.playPosition;
