import 'package:podcast_player/bloc/bloc.dart';
...
class PodcastBloc extends Bloc {
  ...
  void _init() {
    ...
    _subscriptions = PublishSubject<List<Podcast>>(
      onListen: _loadSubscriptions,
    );
  ...
  bool _shouldAutoRefresh() {
    ...
    if (settingsService.autoUpdateEpisodePeriod == -1) {
      ...
      var currentTime = DateTime.now().subtract(
        Duration(minutes: settingsService.autoUpdateEpisodePeriod),
      );
      ...
      if (loadedPodcast != null) {
        ...
        if (_lastFeed.podcast.url == _podcast?.url) {
          ...
          if (feed.podcast.id == null) {
            ...
            _refresh();
            _podcastStream.sink.add(
              BlocPopulatedState<Podcast>(results: _podcast),
            );
          } else {
  ...
  void _listenDownloads() {
    ...
    MobileDownloadService.downloadProgress.listen((downloadProgress) {
      downloadService.findEpisodeByTaskId(downloadProgress.id).then((
        downloadable,
      ) {
      ...
      switch (event) {
        ...
        case PodcastEvent.episodeFilterNone:
          if (_podcast != null) {
            ...
            _podcast = await podcastService.save(
              _podcast!,
              withEpisodes: false,
            );
  ...
  void _listenPodcastSearchEvents() {
    _podcastSearchEvent.debounceTime(const Duration(milliseconds: 200)).listen((
      search,
    ) {
      _searchTerm = search;
      applySearchFilter();
