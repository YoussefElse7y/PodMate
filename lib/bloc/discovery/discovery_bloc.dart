...
import 'package:podcast_player/bloc/discovery/discovery_state_event.dart';
...
class DiscoveryBloc extends Bloc {
  ...
  void _init() {
    _discoveryResults = _discoveryInput.switchMap<DiscoveryState>(
      (DiscoveryEvent event) => _charts(event),
    );
  ...
  Stream<DiscoveryState> _charts(DiscoveryEvent event) async* {
    ...
    if (event is DiscoveryChartEvent) {
      if (_resultsCache == null ||
        ...
        if (_lastIndex > 0) {
          _selectedGenre.add(
            SelectedGenre(index: _lastIndex, genre: _lastGenre),
          );
        } else {
...
class SelectedGenre {
  ...
  SelectedGenre({required this.index, required this.genre});
}
