...
import 'package:podcast_player/core/environment.dart';
import 'package:podcast_player/core/utils.dart';
import 'package:podcast_player/entities/chapter.dart';
import 'package:podcast_player/entities/downloadable.dart';
import 'package:podcast_player/entities/episode.dart';
import 'package:podcast_player/entities/persistable.dart';
import 'package:podcast_player/entities/sleep.dart';
...
import 'package:podcast_player/services/settings/settings_service.dart';
import 'package:podcast_player/state/episode_state.dart';
import 'package:podcast_player/state/persistent_state.dart';
import 'package:podcast_player/state/queue_event_state.dart';
import 'package:podcast_player/state/transcript_state_event.dart';
...
class DefaultAudioPlayerService extends AudioPlayerService {
  ...
  Future<void> seek({required int position}) async {
    ...
    var complete = p.inSeconds > 0
        ? (duration.inSeconds / p.inSeconds) * 100
        : 0;
  ...
  void updateCurrentPosition(Episode? e) {
    if (e != null) {
      ...
      var complete = e.position > 0
          ? (duration.inSeconds / e.position) * 100
          : 0;
  ...
  void _loadEpisodeAncillaryItems() async {
    ...
    if (_currentEpisode!.hasTranscripts) {
      ...
      if (_currentEpisode!.streaming) {
        var sub = _currentEpisode!.transcriptUrls.firstWhereOrNull(
          (element) => element.type == TranscriptFormat.vtt,
        );
        sub ??= _currentEpisode!.transcriptUrls.firstWhereOrNull(
        ...
        if (sub != null) {
          ...
          transcript = await podcastService.loadTranscriptByUrl(
            transcriptUrl: sub,
          );
        ...
        transcript = await repository.findTranscriptById(
          _currentEpisode!.transcriptId!,
        );
      }
