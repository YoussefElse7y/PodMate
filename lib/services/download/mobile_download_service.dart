...
import 'package:podcast_player/services/download/download_manager.dart';
    ...
    try {
      ...
      if (await hasStoragePermission()) {
        ...
        if (episode.hasChapters && episode.chaptersUrl != null) {
          var chapters = await podcastService.loadChaptersByUrl(
        ...
        if (episode.hasTranscripts) {
          ...
          if (sub != null) {
            var transcript = await podcastService.loadTranscriptByUrl(
              transcriptUrl: sub,
            );
        ...
        final downloadPath = await resolveDirectory(
          episode: episode,
          full: true,
        );
        ...
        var filename = safeFile(
          uri.pathSegments.lastWhereOrNull(
