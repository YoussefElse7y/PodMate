...
import 'package:podcast_player/entities/funding.dart';
import 'package:podcast_player/entities/person.dart';
import 'package:podcast_player/entities/podcast.dart';
import 'package:podcast_player/entities/transcript.dart';
import 'package:podcast_player/l10n/messages_all.dart';
...
class MobilePodcastService extends PodcastService {
  ...
  Future<void> _init() async {
    ...
    if (!supportedLocale) {
      for (var l in systemLocales) {
        supportedLocale = await initializeMessages(
    ...
    settingsService.settingsListener.where((event) => event == 'search').listen(
      (event) {
        _setupGenres(currentLocale);
      },
    );
  ...
  Future<Podcast?> loadPodcast({
    ...
    if (fetch) {
      ...
      final existingEpisodes = await repository.findEpisodesByPodcastGuid(
        sourceUrl,
      );
      ...
      for (final episode in loadedPodcast.episodes) {
        final existingEpisode = existingEpisodes.firstWhereOrNull(
      ...
      for (final episode in existingEpisodes) {
        var feedEpisode = loadedPodcast.episodes.firstWhereOrNull(
          (ep) => ep.guid == episode!.guid,
        );
  ...
  Future<Podcast?> subscribe(Podcast? podcast) async {
    ...
    if (podcast != null && podcast.guid != null) {
      var savedEpisodes = await repository.findEpisodesByPodcastGuid(
        podcast.guid!,
      );

      if (podcast.episodes.isNotEmpty) {
        for (var episode in podcast.episodes) {
          var savedEpisode = savedEpisodes.firstWhereOrNull(
            (ep) => ep!.guid == episode.guid,
          );
