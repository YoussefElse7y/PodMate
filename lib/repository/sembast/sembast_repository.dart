...
class SembastRepository extends Repository {
  ...
  SembastRepository({bool cleanup = true, String databaseName = 'anytime.db'}) {
    _databaseService = DatabaseService(
      databaseName,
      version: 2,
      upgraderCallback: dbUpgrader,
    );
  ...
  Future<List<Podcast>> subscriptions() async {
    ...
    final finder = Finder(sortOrders: [titleSortOrder]);

    final List<RecordSnapshot<int, Map<String, Object?>>> subscriptionSnapshot =
        await _podcastStore.find(await _db, finder: finder);
  ...
  Future<List<Episode>> findAllEpisodes() async {
    final finder = Finder(sortOrders: [SortOrder('publicationDate', false)]);
  ...
  Future<List<Episode>> findEpisodesByPodcastGuid(
    ...
    final finder = Finder(filter: episodeFilter, sortOrders: [sortOrder]);
    ...
    if (episode.pguid != null) {
      ...
      if (podcast != null) {
        ...
        if (podcast.filter == PodcastEpisodeFilter.none) {
          ...
          final matchedEpisode = podcast.episodes.indexWhere(
  ...
  Future<void> _upgradeV2(Database db) async {
    List<RecordSnapshot<int, Map<String, Object?>>> data = await _podcastStore
        .find(db);
    ...
    for (var podcast in podcasts) {
      if (podcast.guid!.startsWith('http:')) {
        ...
        final episodes = episodeData
            .map((e) => Episode.fromMap(e.key, e.value))
            .toList();
