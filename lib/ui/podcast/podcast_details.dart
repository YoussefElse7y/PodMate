...
import 'package:podcast_player/entities/feed.dart';
...
import 'package:podcast_player/ui/podcast/funding_menu.dart';
import 'package:podcast_player/ui/podcast/playback_error_listener.dart';
import 'package:podcast_player/ui/podcast/podcast_context_menu.dart';
import 'package:podcast_player/ui/podcast/podcast_episode_list.dart';
import 'package:podcast_player/ui/widgets/action_text.dart';
import 'package:podcast_player/ui/widgets/delayed_progress_indicator.dart';
import 'package:podcast_player/ui/widgets/episode_filter_selector.dart';
import 'package:podcast_player/ui/widgets/episode_sort_selector.dart';
import 'package:podcast_player/ui/widgets/placeholder_builder.dart';
import 'package:podcast_player/ui/widgets/platform_back_button.dart';
import 'package:podcast_player/ui/widgets/platform_progress_indicator.dart';
import 'package:podcast_player/ui/widgets/podcast_html.dart';
import 'package:podcast_player/ui/widgets/podcast_image.dart';
import 'package:podcast_player/ui/widgets/sync_spinner.dart';
...
class _PodcastDetailsState extends State<PodcastDetails> {
  ...
  void initState() {
    ...
    widget._podcastBloc.load(
      Feed(podcast: widget.podcast, backgroundFetch: true, errorSilently: true),
    );
  ...
  void didChangeDependencies() {
    _systemOverlayStyle = SystemUiOverlayStyle(
      statusBarIconBrightness: Theme.of(context).brightness == Brightness.light
          ? Brightness.dark
          : Brightness.light,
      statusBarColor: Theme.of(context).appBarTheme.backgroundColor!.withValues(
        alpha: toolbarCollapsed ? 1.0 : 0.5,
      ),
    );
  ...
  Future<void> _handleRefresh() async {
    ...
    widget._podcastBloc.load(Feed(podcast: widget.podcast, forceFetch: true));
  }
                      ...
                      if (state is BlocLoadingState) {
                        return const SliverToBoxAdapter(
                          child: Padding(
                            ...
                            child: Column(
                              children: <Widget>[PlatformProgressIndicator()],
                            ),
                          ),
                        );
                      }
...
class PodcastHeaderImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (podcast.imageUrl == null || podcast.imageUrl!.isEmpty) {
      return const SizedBox(height: 560, width: 560);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<BlocState<Podcast>>(
      ...
      builder: (context, snapshot) {
        ...
        return EpisodeFilterSelectorWidget(podcast: podcast);
      },
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<BlocState<Podcast>>(
      ...
      builder: (context, snapshot) {
        ...
        return EpisodeSortSelectorWidget(podcast: podcast);
      },
    );
  }
}
