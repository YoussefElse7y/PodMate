...
import 'package:podcast_player/state/queue_event_state.dart';
import 'package:podcast_player/ui/podcast/episode_details.dart';
import 'package:podcast_player/ui/podcast/transport_controls.dart';
import 'package:podcast_player/ui/widgets/action_text.dart';
                  ...
                  child: Column(
                    children: <Widget>[
                      ...
                      ExcludeSemantics(
                        child: Text(
                          widget.queued
                              ? L.of(context)!.queue_remove_label
                              : L.of(context)!.queue_add_label,
                          textAlign: TextAlign.center,
                          style: const TextStyle(fontWeight: FontWeight.normal),
                        ),
                      ),
                ...
                child: TextButton(
                  ...
                  onPressed: () {
                    showModalBottomSheet<void>(
                      ...
                      builder: (context) {
                        return EpisodeDetails(episode: widget.episode);
                      },
                    );
                  },
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: <Widget>[
                      const Icon(Icons.unfold_more_outlined, size: 22),
                      const Padding(
                        padding: EdgeInsets.symmetric(vertical: 2.0),
                      ),
                      Text(
...
class _CupertinoAccessibleEpisodeTile extends StatefulWidget {
  ...
  @override
  State<_CupertinoAccessibleEpisodeTile> createState() =>
      _CupertinoAccessibleEpisodeTileState();
}

class _CupertinoAccessibleEpisodeTileState
    extends State<_CupertinoAccessibleEpisodeTile> {
  bool expanded = false;
    ...
    return StreamBuilder<_PlayerControlState>(
      stream: Rx.combineLatest2(
        ...
        (AudioState audioState, Episode? episode) =>
            _PlayerControlState(audioState, episode),
      ),
      builder: (context, snapshot) {
        ...
        final currentlyPlaying =
            nowPlaying?.guid == widget.episode.guid &&
            audioState == AudioState.playing;
        final currentlyPaused =
            nowPlaying?.guid == widget.episode.guid &&
                ...
                builder: (BuildContext context) {
                  return CupertinoActionSheet(
                    actions: <Widget>[
                      ...
                      if (widget.episode.downloadState ==
                              DownloadState.queued ||
                          widget.episode.downloadState ==
                      ...
                      CupertinoActionSheetAction(
                        ...
                        child: Text(
                          L.of(context)!.episode_details_button_label,
                        ),
                      ),
                    ],
                    ...
                    children: <Widget>[
                      ...
                      if (!currentlyPlaying &&
                        ...
                        SimpleDialogOption(
                          ...
                          child: Text(
                            L.of(context)!.play_download_button_label,
                          ),
                        ),
                      if (!currentlyPlaying &&
                          !currentlyPaused &&
...
class EpisodeTransportControls extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (download) {
      buttons.add(
        Semantics(container: true, child: DownloadControl(episode: episode)),
      );
    }

    if (play) {
      buttons.add(
        Semantics(container: true, child: PlayControl(episode: episode)),
      );
    }
...
class EpisodeSubtitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (episode.publicationDate != null) {
      ...
      if (diff.inDays < 7) {
        (dateLabel, dateSemanticLabel) = calculateTimeAgo(
          context,
    ...
    if (length.inSeconds > 0) {
      if (length.inSeconds < 60) {
        ...
        semanticTitle =
            '$dateSemanticLabel, ${L.of(context)!.time_semantic_seconds(length.inSeconds)}';
      } else {
        title = '$dateLabel • ${L.of(context)!.time_minutes(length.inMinutes)}';
        semanticTitle =
            '$dateSemanticLabel, ${L.of(context)!.time_semantic_minutes(length.inMinutes)}';
      }
    } else {
      title = dateLabel;
    ...
    if (timeRemaining.inSeconds > 0) {
      if (timeRemaining.inSeconds < 60) {
        title =
            '$title / ${L.of(context)!.episode_time_second_remaining(timeRemaining.inSeconds.toString())}';
        semanticTitle =
            '$semanticTitle / ${L.of(context)!.episode_semantic_time_second_remaining(timeRemaining.inSeconds.toString())}';
      } else {
        title =
            '$title / ${L.of(context)!.episode_time_minute_remaining(timeRemaining.inMinutes.toString())}';
        semanticTitle =
  ...
  (String, String) calculateTimeAgo(
    ...
    if ((difference.inDays / 7).floor() >= 1) {
      ...
      semanticLabel = L
          .of(context)!
          .episode_semantic_time_days_ago(difference.inDays);
    } else if (difference.inHours >= 1) {
      label = L.of(context)!.episode_time_hours_ago(difference.inHours);
      semanticLabel = L
          .of(context)!
          .episode_semantic_time_hours_ago(difference.inHours);
    } else if (difference.inMinutes >= 1) {
      label = L.of(context)!.episode_time_minutes_ago(difference.inMinutes);
      semanticLabel = L
          .of(context)!
          .episode_semantic_time_minutes_ago(difference.inMinutes);
    } else {
